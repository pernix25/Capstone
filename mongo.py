...
def populate_db():
    with open('holds_classification.txt', 'r') as fyle:
        text_data = fyle.readlines()[1:]

    for r_index, row in enumerate(text_data):
        row = row.strip().split(',')[1:]

        for c_index, hold in enumerate(row):
            pos = (r_index, c_index)

            collection.insert_one({'row': r_index, 'col': c_index, 'hold_type': hold, 'img_coords': (hold_coords[r_index][c_index])})
